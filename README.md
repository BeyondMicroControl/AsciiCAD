...
Apart from demostrating it's higher purpose, the product of this, covers a real need: **embedding digital and electronic schematics directly inside source code**. Whereas microcontroller projects traditionally require different CAD files revolving around source code, AsciiCAD schematics made of **readable text** can live, evolve the nearest it can be to its codebase. **Caution**: AsciiCAD does not replace CAD apps for driving an entire PCB design workflow end-to-end; it is most effective in speeding up circuit prototyping.  **AsciiCAAD enables uniting codebase and schematic as one**.
