...
**Caution**: AsciiCAD does not replace the need for a real CAD program for electronic schema design, but when downloading (e.g. Arduino) source code, a simple embedded schematic enables fast prototyping and getting its associated code running in no time and without obstacles .
