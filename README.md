...
Moreover, not only does AsciiCAD depict an entire schema visually; its very same 2D arrangement holds enough data density to retain sufficient metadata & syntactics of a wiring diagram allowing logical and even electric behavior simulation (e.g. SPICE), literally by pasting "just text" into a grid.  It would be a cleaner approach if we'd keep this project domain-agnostic (e.g. electronics), fortunately, we clearly define and consciously keep [specialisation layers](https://github.com/BeyondMicroControl/AsciiCAD/blob/main/docs/design.md#specialisation-layers) apart, allowing anyone to fork this project for other uses in Computer Aided Design.
