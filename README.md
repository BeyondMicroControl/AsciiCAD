...
This is a browser-based app using building blocks once raised to the level of art: **ASCII**.  The project was mainly designed as an essay on fasttracking complexity by strategising bare-bones simplicity at its conception, and letting [policy](docs/design.md) and Large Language Models (LLMs) do the heavy lifting.  As a matter of fact, v1.0 was put together in a few weeks’ time.

The main thesis of this project revolves around **2D semantics**.  While LLMs remain champions at making sense out of plain text, if presented as a one-dimensional array of words or 'tokens', LLMs come extremely unprepaired for two-dimensional semantics.  It's all well for humans to interact with LLMs using natural language to express questions, reason across multiple subject matters and even provide training data we usually call AI memory.  But here's the thing: humans developed language in a multi-modal way, like images, speech, music, poetry, even ASCII-art are modes LLMs heavily struggle with.   While many of these modes rely on meaning extraction from very large and complex amounts of data, Ascii tables, schemas and diagrams remain extremely light, although still give an [extremely hard time for LLMs to decode](https://www.youtube.com/watch?v=rWtMkoRhm-M).  That's exactly what we are trying to find out.
