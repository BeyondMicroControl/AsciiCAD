...
The main thesis of this project revolves around **2D semantics**.  While LLMs remain champions at making sense out of plain text, if presented as a one-dimensional array of words or 'tokens', LLMs come extremely unprepaired for two-dimensional semantics.  It's all well for humans to interact with LLMs using natural language to express questions, reason, and even provide training data we call AI memory.  But here's the thing: humans developed language in a multi-modal way, like images, speech, music, poetry, even ASCII-art are modes LLMs heavily struggle with.   While many of these modes rely on meaning extraction from very large formatted datasets, Ascii tables, schemas and diagrams remain extremely light, although still give an [extremely hard time for LLMs to decode](https://www.youtube.com/watch?v=rWtMkoRhm-M).  That's exactly what we are trying to find out and perhaps solve.
