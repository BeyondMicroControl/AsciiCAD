...
this.interpolateColors = function(col_arr, rangeLen) 
{
  ...
  this.toArray = function(v)
  {
    if (v == null) return [];
    return Array.isArray(v) ? v.slice() : [v];
  }

  this.markdownEscape = function(str)
  {
    return String(str ?? "")
      .replace(/\\/g, "\\\\")
      .replace(/\|/g, "\\|")
  ...
  this.normalizeHelpEntry = function(name, help)
  {
    ...
    const parameters = this.toArray(help.parameters).map((p) =>
    {
      ...
      if (Array.isArray(p))
        return { name: String(p[0] ?? ""), description: String(p[1] ?? "") };
    ...
    {
      if (typeof ex === "string")
        return { title: "", code: ex, description: "" };

      if (Array.isArray(ex))
        return {
          title: String(ex[0] ?? ""),
          code: String(ex[1] ?? ""),
          description: String(ex[2] ?? "")
        };

      return {
        title: String(ex?.title ?? ex?.caption ?? ""),
        code: String(ex?.code ?? ex?.cmd ?? ""),
        description: String(ex?.description ?? ex?.desc ?? "")
      };
    });
    ...
    return {
      kind: String(help.kind || help.type || "command"),
      title: String(help.title || name),
      summary: String(help.summary || help.desc || ""),
      appliesTo: this.toArray(help.appliesTo).map(String),
      syntax,
      parameters,
      remarks,
      effects,
  ...
  this.helpEntryToMarkdown = function(name, help, opts)
  {
    opts = opts || {};
    const h = this.normalizeHelpEntry(name, help);
    const out = [];

    const headingLevel = Math.max(1, Number(opts.headingLevel || 1));
    ...
    {
      out.push("", H2 + " Syntax", "");
      for (let i = 0; i < h.syntax.length; i++)
      {
        out.push("```txt");
        out.push(String(h.syntax[i]));
        out.push("```");
      }
    }
    ...
    if (h.parameters.length)
    {
      ...
      for (let i = 0; i < h.parameters.length; i++)
      {
        ...
        out.push(
          "| " +
          this.markdownEscape(p.name) +
          " | " +
          this.markdownEscape(p.description) +
          " |"
        );
      }
    }
    ...
    {
      out.push("", H2 + " Remarks", "");
      for (let i = 0; i < h.remarks.length; i++)
        out.push("- " + h.remarks[i]);
    }

    if (h.examples.length)
    {
      out.push("", H2 + " Examples", "");
      for (let i = 0; i < h.examples.length; i++)
      {
        const ex = h.examples[i];
    ...
    {
      out.push("", H2 + " Additional references", "");
      for (let i = 0; i < h.references.length; i++)
      {
        const ref = h.references[i];
        if (typeof ref === "string") out.push("- " + ref);
        else if (ref && ref.href) out.push("- [" + ref.title + "](" + ref.href + ")");
  ...
  this.JShelpMarkdownCollector = function(JSContainer, param)
  {
    param = param || {};
    ...
    return entries.map(e => e.markdown).join("\n\n---\n\n");
  };
