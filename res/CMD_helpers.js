...
worker.addEventListener("message", (e) => {
  ...
  if (msg.type === "call") {
    ...
    try {
      if (typeof oASC[name] !== "function") throw new Error("No such method: " + name);
      const result = oASC[name](...(args || []));
      worker.postMessage({ type: "ret", id, ok: true, result });
    } catch (err) {
      worker.postMessage({ type: "ret", id, ok: false, error: String(err?.message || err) });
    }
    return;
  }

  // done/error are handled per-run in runExternalScript() below
});
