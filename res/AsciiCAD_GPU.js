...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    this.routePathDijkstraCanPass = function(codeA, codeB, dir)
    {
        const N = 1, E = 2, S = 4, W = 8;
        codeA = codeA | 0;
        codeB = codeB | 0;
        ...
        if (dir === N || dir === S)
            return ((codeA === 1 || codeA === 2) && (codeB === 1 || codeB === 2));

        if (dir === E || dir === W)
            return ((codeA === 1 || codeA === 3) && (codeB === 1 || codeB === 3));
    ...
    this.routePathDijkstraBacktrace16 = function(distState, mask16, bannedBits, from, to, modifiers)
    {
        //const N = 1, E = 2, S = 4, W = 8;

        const dist2D = (distState && typeof distState.toArray === "function") ? distState.toArray() : distState;
        if (!dist2D || !mask16) return null;
        ...
        while ((curR !== from.r || curC !== from.c) && guard-- > 0)
        {
            ...
            const curCode = this.routeCodeAtMask16(mask16, curIdx, srcIdx, dstIdx, bannedBits);
            ...
            for (let i = 0; i < order.length; i++)
            {
                ...
                const prevIdx = nr * COLS + nc;
                const prevCode = this.routeCodeAtMask16(mask16, prevIdx, srcIdx, dstIdx, bannedBits);
                if (!this.routePathDijkstraCanPass(prevCode, curCode, n.dir)) continue;
    ...
    this.routePathDijkstra = function(from, to, modifiers)
    {
        ...
        if (!oASC || typeof oASC.routeBuildContextGPU !== "function")
            return oCOM.debugDone(null, "GPU routePathDijkstra()", "missing-routeBuildContextGPU");
        ...
        if (!mask16) return oCOM.debugDone(null, "GPU routePathDijkstra()", "mask16-failed");

        const cfg16 = new Uint16Array(7);
        cfg16[0] = COLS;
        cfg16[1] = ROWS;
        cfg16[2] = from.c;
        cfg16[3] = from.r;
        cfg16[4] = to.c;
        cfg16[5] = to.r;
        cfg16[6] = this.DIJKSTRA_INF;

        const okInit = this.runGPU(
            this.routePathDijkstraInit16,
            { mode: "gpu" },
            {
                output: [COLS, ROWS],
                precision: "single",
        ...
        if (okInit === false) return oCOM.debugDone(null, "GPU routePathDijkstra()", "kernel-init16-compile-failed");

        const okStep = this.runGPU(
        this.routePathDijkstraStep16,
            { mode: "gpu" },
            {
                output: [COLS, ROWS],
                precision: "single",
            ...
            { distPrev: [ [[0]] ], mask16: [mask16], bannedBits: [bannedBits], cfg16: [cfg16] }
            );
        if (okStep === false) return oCOM.debugDone(null, "GPU routePathDijkstra()", "kernel-step16-compile-failed");

        const okProbe = this.runGPU(
            this.routePathDijkstraProbe16,
            { mode: "gpu" },
            {
                output: [1],
                precision: "single",
            ...
            { distState: [ [[0]] ], cfg16: [cfg16] }
            );
        if (okProbe === false) return oCOM.debugDone(null, "GPU routePathDijkstra()", "kernel-probe16-compile-failed");

        try
        {
            let dist = this.routePathDijkstraInit16.kObject(mask16, bannedBits, cfg16);
            let targetDist = this.routePathDijkstraProbe16.kObject(dist, cfg16)[0];

            for (let iter = 0; iter < (ROWS * COLS); iter++)
            {
                if (Number.isFinite(targetDist) && targetDist < this.DIJKSTRA_INF) break;
                dist = this.routePathDijkstraStep16.kObject(dist, mask16, bannedBits, cfg16);
                targetDist = this.routePathDijkstraProbe16.kObject(dist, cfg16)[0];
            }
    ...
    this.routePathDijkstraInit16 = function() {};
    this.routePathDijkstraInit16.kObject = null;
    this.routePathDijkstraInit16.kScript = function(mask16, bannedBits, cfg16)
    {
    ...
    this.routePathDijkstraStep16 = function() {};
    this.routePathDijkstraStep16.kObject = null;
    this.routePathDijkstraStep16.kScript = function(distPrev, mask16, bannedBits, cfg16)
    {
        const cols = cfg16[0] | 0;
        const rows = cfg16[1] | 0;
        const srcC = cfg16[2] | 0;
        const srcR = cfg16[3] | 0;
        const dstC = cfg16[4] | 0;
        const dstR = cfg16[5] | 0;
        const INF  = cfg16[6] | 0;
        ...
        if (r > 0)
        {
            ...
            {
                nIdx = (r - 1) * cols + c;
                nCode = (mask16[nIdx] >>> 8) & 3;
                if (nIdx === srcIdx || nIdx === dstIdx) nCode = 1;
                else if ((bannedBits[nIdx] | 0) > 0) nCode = 0;

                if ((code === 1 || code === 2) && (nCode === 1 || nCode === 2))
                {
                    cand = cand + 1;
        ...
        if (c > 0)
        {
            ...
            if (cand < INF)
            {
                ...
                if ((code === 1 || code === 3) && (nCode === 1 || nCode === 3))
                {
                    cand = cand + 1;
                    if (cand < best) best = cand;
                }
...
this.routePathDijkstraProbe16 = function() {};
this.routePathDijkstraProbe16.kObject = null;
this.routePathDijkstraProbe16.kScript = function(distState, cfg16)
{
    const toC = cfg16[4] | 0;
    const toR = cfg16[5] | 0;
    return distState[toR][toC];
};
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    {
        oCOM.startChrono("oGASC.mikamiPath");
    ...
    this.mikamiPathClearH = function(mask16, bannedBits, srcIdx, dstIdx, r, c0, c1)
    {
        ...
        for (let c = c0 + step; c !== c1 + step; c += step)
        {
            ...
            if(code !== 1 && code !== 3) return false;      // free / terminal OR wire_v bridge cell
        }
        return true;
    };
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    this.mikamiInitH.kScript = function(mask16, bannedBits, cfg16)
    {
        ...
        for (let cc = srcC + step; cc !== c + step; cc += step)
        {
    ...
    this.mikamiInitV.kScript = function(mask16, bannedBits, cfg16)
    {
        ...
        for (let rr = srcR + step; rr !== r + step; rr += step)
        {
            const kIdx = rr * cols + c;
            let k = (mask16[kIdx] >>> 8) & 3;
            if (kIdx === srcIdx || kIdx === dstIdx) k = 1;
            else if ((bannedBits[kIdx] | 0) > 0) k = 0;
